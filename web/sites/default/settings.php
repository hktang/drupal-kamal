...
if (getenv('KAMAL_HOST')) {
   ...
   * and .kamal/secrets files (and their staging/development variations).
   */
  $databases['default']['default'] = [
 ...
 * @endcode
 */
$settings['hash_salt'] = getenv('DRUPAL_HASH_SALT') ?: '21dd24dfb098ccd8766dc4027e7accbf';
...
$settings['trusted_host_patterns'] = [
  '^.+\.localhost$',
  # Use the APP_DOMAIN environment variable to set the trusted host patterns.
  getenv('APP_DOMAIN') ? '^' . preg_quote(getenv('APP_DOMAIN')) . '$' : '',
  getenv('APP_DOMAIN') ? '^.+\.' . preg_quote(getenv('APP_DOMAIN')) . '$' : '',
];
...
// Load Kamal-related settings.
if (getenv('KAMAL_HOST') && file_exists(__DIR__ . '/settings.kamal.php')) {
  include __DIR__ . '/settings.kamal.php';
}
/**
